...
def detect_lidar_msg_type(bag_file):
    ...
    返回：
        ("PointCloud2", topic_name) 或 ("CustomMsg", topic_name) 或 (None, None)
    ...
    if pc2_topic and livox_topic:
        print(f"[Detect] 同时检测到 PointCloud2 ({pc2_topic}) 和 Livox CustomMsg ({livox_topic}), 默认使用 PointCloud2。")
        return "PointCloud2", pc2_topic
    elif pc2_topic:
        print(f"[Detect] 检测到 PointCloud2 点云, topic: {pc2_topic}")
        return "PointCloud2", pc2_topic
    elif livox_topic:
        print(f"[Detect] 检测到 Livox CustomMsg 点云, topic: {livox_topic}")
...
def select_and_save_points(pcd_folder, target_pcd_name):
    ...
    # --- 下采样优化 ---
    print(f"[Process] 原始点数: {len(pcd.points)}")
    voxel_size = 0.05  # 5cm 体素下采样
    print(f"[Process] 执行体素下采样 (voxel_size={voxel_size}m)...")
    pcd = pcd.voxel_down_sample(voxel_size=voxel_size)
    print(f"[Process] 下采样后点数: {len(pcd.points)}")
    # ------------------
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 2:
        ...
        print(f"未指定输出目录，使用 bag 文件所在目录: {output_dir}")
